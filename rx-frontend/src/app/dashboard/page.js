        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-1.5 text-sm text-slate-500">
              Today&apos;s orders, performance and activity across all branches.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              href="/dashboard/fulfillment"
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-xl shadow-sm hover:bg-slate-50 btn-press transition focus-ring"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-teal-500 to-emerald-500 rounded-xl shadow-lg shadow-teal-500/25 hover:from-teal-600 hover:to-emerald-600 btn-press transition focus-ring"
            >
       ...
       <DashboardCards
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="Add your first pharmacy branch to see order activity."
            variant="info"
            ctaHref="/branches"
            ctaLabel="Go to Branches"
          />
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 stagger">
            {branches.map((branch, idx) => (
              <section
                key={branch.branchId}
                className={`bg-white rounded-2xl border border-slate-200 shadow-sm card-hover overflow-hidden animate-fade-in-up`}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {branch.orderCount || 0} orders today
                      </p>
                    </div>
                  </div>
                  <Link
                    href={`/dashboard/stock`}
                    className="text-xs font-medium text-teal-700 hover:text-teal-800 px-3 py-1.5 rounded-lg bg-teal-50 hover:bg-teal-100 transition-colors"
                  >
                    View stock →
                  </Link>
                </header>
                ...
                {(branch.orders || []).length === 0 ? (
                  <div className="px-5 py-10">
                    <EmptyState
                      ...
                      description="Once orders are placed they will appear here."
                      size="sm"
                    />
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    ...
                    {branch.orders.length > 5 && (
                      <div className="px-5 py-3 border-t border-slate-100 text-xs text-slate-500 bg-slate-50/30">
                        Showing 5 of {branch.orders.length} orders
                      </div>
                    )}
                  </div>
                )}
